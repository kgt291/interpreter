...
W_fc1 = tf.Variable(tf.zeros([784, 1024])) 
b_fc1 = tf.Variable(tf.zeros([1024]))

h_fc1 = tf.sigmoid(tf.matmul(x, W_fc1) + b_fc1) #sigmoid (hidden layer)

W_fc2 = tf.Variable(tf.zeros([1024, 256]))
b_fc2 = tf.Variable(tf.zeros([256]))

h_fc2 = tf.sigmoid(tf.matmul(h_fc1, W_fc2) + b_fc2)

W_fc3 = tf.Variable(tf.zeros([256, 10]))
