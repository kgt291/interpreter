...
void syntaxChk()
{
	...
	for (Pc = 1; Pc < (int)intercode.size(); Pc++) {
		...
		case For:
			...
			if (code.kind == Step) (void)get_expression(0, EofLine);
			chk_EofLine();
			break;
		case Fcall:
...
void binaryExpr(TknKind op)
{
	...
	if ((op == Divi || op == Mod || op == IntDivi) && d2 == 0) err_exit("0으로 나누었습니다.");
...
void fncCall_syntax(int fncNbr)
{
	...
	if (argCt != Gtable[fncNbr].args) err_exit(Gtable[fncNbr].name, "함수의 인수 개수가 잘못되었습니다.");
	stk.push(1.0);
}
...
void sysFncExec_syntax(TknKind kd)
{
	...
	case Input:
		code = nextCode();
		code = chk_nextCode(code, '('); code = nextCode();
		stk.push(1.0);
		break;
	case Print: case Println:
		do {
