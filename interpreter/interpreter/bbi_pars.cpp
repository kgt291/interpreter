...
void convert()
{
	...
	case If:
		...
		break;
	case Break:
		if (loopNest <= 0) err_exit("잘못된 break 입니다.");
		setCode(token.kind); token = nextTkn(); convert_rest();
		break;
	case Return:
		if (!fncDecl_F) err_exit("잘못된 return 입니다.");
		setCode(token.kind); token = nextTkn(); convert_rest();
		break;
	case Exit:
		setCode(token.kind); token = nextTkn(); convert_rest();
	...
	for (;;) {
		...
		case Ident: 
			...
			if ((tblNbr = searchName(tmpTb.name, 'F')) != -1) {
				if (tmpTb.name == "main") err_exit("main 함수는 호출할 수 없습니다.");
...
void var_namechk(const Token& tk)
{
	...
	if (is_localScope() && tk.text[0] == '$') err_exit("함수 내 var선업ㅇ서는 $가 붙은 이름을 지정할 수 없습니다.: ", tk.text);
	if (searchName(tk.text, 'V') != -1) err_exit("식별자가 중복되었습니다.: ", tk.text);
}

void set_name()
{
	if (token.kind != Ident) err_exit("식별자가 필요합니다.:", token.text);
	tmpTb.clear(); tmpTb.name = token.text; token = nextTkn();
}
...
void set_aryLen()
{
	...
	token = nextTkn();
	if (token.kind != IntNum) err_exit("배열 길이는 양(+)의 정수로 지정해주세요.: ", token.text);
...
void fncDecl()
{
	...
	if (blkNest > 0) err_exit("함수 정의의 위치가 바르지 않습니다.");
	fncDecl_F = true;
	localAdrs = 0;
	set_startLtable();
...
void push_intercode()
{
	...
	*codebuf_p++ = '\0';
	if ((len = codebuf_p - codebuf) >= LIN_SIZ) err_exit("변환 후의 내부 코드가 너무 깁니다. 식을 줄여주세요.");
