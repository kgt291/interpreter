...
enum TknKind {
	Lparen=1, Rparen, Plus, Minus, Multi, Divi, Assign, Comma, DbIQ, Equal, NotEq, Less, LessEq, Great, GreatEq,
...
int main(int argc, char* argv[]) {
	...
	for (token = nextTkn(); token.kind != EofTkn; token = nextTkn()) {
		cout << left << setw(10) << token.text << right << setw(3) << token.kind << " " << token.intVal << endl;
	}
	return 0;
}
